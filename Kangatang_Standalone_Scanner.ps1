    ...
    try {
        ...
        if ($isInfected) {
            ...
            try {
                ...
                try { Set-ItemProperty -LiteralPath $backupPath -Name IsReadOnly -Value $false -ErrorAction SilentlyContinue } catch {}
                if ($isCentral) {
                    Write-Host "     📦 [SAO LƯU HUB] Đã cách ly file nhiễm về Hub tập trung: $backupName" -ForegroundColor Green
                    Write-AuditLog "[BACKUP_CENTRAL] $backupPath"
                } else {
                    Write-Host "     📦 [SAO LƯU OFFLINE] Mạng LAN offline, đã lưu cách ly cục bộ tại: $backupName" -ForegroundColor DarkYellow
